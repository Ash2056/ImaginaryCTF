...
class VM:
    ...
    def debug(self, *args):
        if args[0] == '"before swap"':
            print(f"pivot = {self.mem.regs['r2']}")
            print(f"i = {self.mem.regs['r4']}")
            print(f"j = {self.mem.regs['r6']}")
            arr = [self.mem.ram[i] for i in range(self.mem.regs["r15"])]
            print(f"{arr = }")
            return
        if args[0] == '"vals"':
            print(args[1])
        ...
        print(f"{self.mem.regs = }")
        print(f"{self.mem.ram = }")
